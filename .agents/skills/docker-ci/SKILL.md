...
Key build args:
- `TAILSCALE_VERSION` (default: `v1.102.4`) — image tag for `ghcr.io/tailscale/tailscale`
...
1. **frontend** — `npm install` + `npm run build` in `webui/frontend/` (Node.js 24.21.0 in CI)
...
| Tailscale | `v1.102.4` (`ghcr.io/tailscale/tailscale` base image) |
| Go | `1.27.1` (Dockerfile ARG) |
| Node.js (CI) | `24.21.0` (GitHub Actions + Dockerfile ARG) |
