...
|-----|--------|-------------|-------------|
| `frontend` | ubuntu-latest | `webui/frontend` | Node 24.21.0 → `npm install` → `npm run build` |
| `backend` | ubuntu-latest | `webui` | Node 24.21.0 + Go 1.27.1 → `npm install` + `npm run build` (for `//go:embed all:web/dist`) → `go vet ./...` → `go test -v ./...` → `go build -v ./...` |
| `integration` | ubuntu-latest | repo root | Node 24.21.0 + Docker Buildx + Python 3.12 → `pytest tests/integration/ -v` |
