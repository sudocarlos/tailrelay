...
Combines **Tailscale VPN**, **Tailscale Serve** (HTTPS + TCP relays),
...
- :material-desktop-classic: **Browser-Based Management** — a responsive Web
  UI running on port `8021` to manage your relays.

- :material-lock-check: **Automatic TLS** — Tailscale Serve terminates TLS for
  HTTPS relays with automatic MagicDNS hostnames.

- :material-swap-horizontal: **HTTPS Relays** — configure HTTPS reverse
  proxies/relays through the UI.

- :material-network: **TCP Relays** — forward non-HTTP protocols through
  Tailscale Serve.

- :material-earth: **Funnel** — expose a service to the public internet on
  port `443`, `8443`, or `10000` via Tailscale Funnel.

- :material-content-save-cog: **Backup & Restore** — save, download, upload,
  and restore configurations.

- :material-shield-key: **Dual Authentication** — authenticate via Tailscale
  network identity (peer IP) or secure token.

- :material-cpu-64-bit: **Multi-Platform** — multi-arch Docker images built
  for `amd64` and `arm64`.
...
container, or jump straight to the [API Reference](api.md).
