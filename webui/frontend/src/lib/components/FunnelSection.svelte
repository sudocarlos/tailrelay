...
<div class="rounded-lg border border-gray-200 dark:border-gray-800 overflow-hidden mb-6">
  <!-- Header / Toggle -->
  <button
  ...
  >
    <div>
      <div class="flex items-center gap-2">
        <Globe size={15} class="text-gray-500 dark:text-gray-400" />
        <span class="text-sm font-medium">Funnel</span>
        {#if funnels.length > 0}
          <span class="text-xs text-gray-400 dark:text-gray-500">{funnels.length}</span>
        {/if}
      </div>
      {#if !expanded}
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5 ml-[23px]">
  ...
  {#if expanded}
    <div class="flex flex-col gap-3 p-4 border-t border-gray-200 dark:border-gray-800">
