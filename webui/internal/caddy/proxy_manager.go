...
// UpdateProxyHostnames replaces every proxy whose Hostname matches oldFQDN
// (case-insensitive) with newFQDN, updating both metadata and the live Caddy
// route. This is called after a Tailscale hostname change so that *.ts.net
// proxy entries don't become stale.
func (pm *ProxyManager) UpdateProxyHostnames(oldFQDN, newFQDN string) error {
	oldFQDN = NormalizeHostname(oldFQDN)
	newFQDN = NormalizeHostname(newFQDN)

	if oldFQDN == "" || newFQDN == "" {
		return fmt.Errorf("UpdateProxyHostnames: oldFQDN and newFQDN must be non-empty")
	}
	if strings.EqualFold(oldFQDN, newFQDN) {
		logger.Debug("caddy", "UpdateProxyHostnames: old and new FQDNs are equal (%s), nothing to do", newFQDN)
		return nil
	}

	proxies, err := LoadProxyMetadata(pm.metadataPath)
	if err != nil {
		return fmt.Errorf("UpdateProxyHostnames: load metadata: %w", err)
	}

	updated := 0
	var firstErr error
	for _, proxy := range proxies {
		if !strings.EqualFold(NormalizeHostname(proxy.Hostname), oldFQDN) {
			continue
		}
		proxy.Hostname = newFQDN
		if err := pm.UpdateProxy(proxy); err != nil {
			logger.Error("caddy", "UpdateProxyHostnames: failed to update proxy %s: %v", proxy.ID, err)
			if firstErr == nil {
				firstErr = err
			}
			continue
		...
		updated++
		logger.Info("caddy", "UpdateProxyHostnames: updated proxy %s from %s to %s", proxy.ID, oldFQDN, newFQDN)
	}

	if updated > 0 {
		logger.Info("caddy", "UpdateProxyHostnames: renamed %d proxy hostname(s) from %s → %s", updated, oldFQDN, newFQDN)
	}
	return firstErr
}
